...
export default function TaxFormViewer({
  ...
  useEffect(() => {
    ...
    const loadPdf = async () => {
      ...
      try {
        ...
        const pdfUrl = `/api/pedidos/${pedidoId}/tax-form-pdf`
        ...
        if (!cancelled) {
          console.error('[TaxFormViewer] Error cargando PDF:', err)
          setPdfError('No se pudo cargar el documento. Inténtalo de nuevo.')
        }
      } finally {
  ...
  const renderPage = useCallback(async (pageNum: number, scaleVal: number) => {
    ...
    try {
      ...
      console.error('[TaxFormViewer] Error renderizando página:', err)
    }
  }, [pdfDoc])
  ...
  const handleApprove = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/pedidos/${pedidoId}/approve-tax-form`, { method: 'POST' })
  ...
  return (
    <div className="mt-6 pt-6 border-t border-slate-100">
      <div className={`rounded-2xl border-2 overflow-hidden transition-all duration-500 ${isApproved
        ...
        <div className="p-6 pb-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-xs font-black uppercase tracking-widest ${isApproved ? 'text-green-700' : 'text-blue-700'
              ...
              {isApproved ? 'Documentos Aprobados' : 'Acción Requerida — Revisa y Aprueba'}
            </span>
          </div>
          <h4 className={`text-xl font-black mb-1 ${isApproved ? 'text-green-900' : 'text-blue-900'}`}>
            📄 Formularios 5472 + 1120
          </h4>
          <p className={`text-sm ${isApproved ? 'text-green-700' : 'text-blue-600'}`}>
            {!isApproved && 'Lee detenidamente el documento. El IRS aplica multas de +$25,000 USD por información incorrecta. Revisa y aprueba a continuación.'}
            {isApproved && !isCompleted && 'Documentos aprobados ✓  Nuestro equipo procederá al envío al IRS en los próximos 1-3 días hábiles.'}
            {isApproved && isCompleted && 'Formularios presentados oficialmente ante el IRS. El trámite ha concluido.'}
          </p>
        </div>
        ...
        {/* ── Canvas: bloqueamos el menú contextual aquí ── */}
        <div
          ref={containerRef}
        ...
        {/* ── Checkbox de aprobación (antes de aprobar) ── */}
        {!isApproved && (
          <div className="mx-4 mb-6 space-y-4 bg-white p-5 rounded-xl border border-blue-100 shadow-sm">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-xs font-bold text-blue-800 uppercase tracking-wide">Declaración Jurada de Exactitud</p>
            </div>
            <label className="flex items-start gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-slate-700 leading-relaxed group-hover:text-slate-900 transition-colors">
                Declaro bajo pena de perjurio que he examinado la información contenida en estos formularios (5472 y 1120) y sus anexos, y que según mi leal saber y entender, es <strong>verdadera, correcta y completa</strong>. Autorizo a Open LLC USA a presentarlos ante el IRS en mi nombre.
              </span>
            </label>
            <button
            ...
            >
              {loading
                ...
                : <><ShieldCheck size={18} /> Confirmar y Aprobar Documentos</>
              }
            </button>
          </div>
        )}
          ...
          <div className="mx-4 mb-6">
            {!isCompleted ? (
              <div className="flex items-center gap-3 px-5 py-3.5 bg-green-100 text-green-800 rounded-xl font-bold border border-green-200 text-sm">
                <Clock size={18} />
                Documentos aprobados — En proceso de envío al IRS
              </div>
            ) : (
              <a
                href={`/api/pedidos/${pedidoId}/tax-form-pdf`}
                download="formulario-5472-1120.pdf"
                className="inline-flex items-center gap-2 px-6 py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 transition-all shadow-lg shadow-green-200 text-sm"
              >
                <CheckCircle2 size={18} />
                Descargar Copia Oficial (IRS confirmado)
              </a>
            )}
          </div>
        )}
